...
tools = toolset.get_tools(actions=[Action.TWITTER_USER_LOOKUP_BY_USERNAME, Action.TWITTER_RECENT_SEARCH, Action.TWITTER_BOOKMARKS_BY_USER, Action.SLACK_SENDS_A_MESSAGE_TO_A_SLACK_CHANNEL, Action.TWITTER_RECENT_SEARCH, Action.TAVILY_TAVILY_SEARCH, Action.FIRECRAWL_SCRAPE_EXTRACT_DATA_LLM])
...
prefix_messages = [
    ChatMessage(
        role="system",
            content=(
            """You are an advanced trend analyzer specializing in AI/Crypto/ Onchain Agents/ DEFAI technology trends.
                Your job is not only to find the trends but also to find the people who are talking about it and moreover based on the trend to derive valuable and actionable insights.
                Based on an idea presented, you will have to think of similar ideas in this field. 
                Remember your goal first of all is to help me as a starup founder stay informed all the time on whats happening and helpe me act on trends & ideas fast.
                You will be given a twitter id and a slack channel id. You will have to send the output on the slack channel.
                Output Format:
...
id = '@@beast_ico' #your twitter id
channel = 'C04GQULAMB5' #your slack channel
